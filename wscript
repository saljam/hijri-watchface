...
sdk_path = os.environ['PEBBLE_HOME'] + "/Pebble/sdk"
...
def build(bld):
    ...
    bld.program(
        source       = sources,
        target       = elf_file,
        includes     = [sdk_path + '/include', '.', 'build/src'],
        ldscript     = bld.root.find_node(sdk_path + '/pebble_app.ld'),
        stlibpath    = [sdk_path + '/lib'],
        stlib        = ['pebble'],
    ...
    bundle_script = bld.root.find_node(sdk_path + '/tools/mkbundle.py')
